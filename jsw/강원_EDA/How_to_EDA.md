...
- **플롯 이미지 분석 원칙 (필수)**: EDA 노트북 내에 시각화 플롯(차트)이 존재할 경우, 단순 수치나 코드 텍스트만으로 해석을 유추하여 작성하는 것은 금지된다. **반드시 직접 플롯 이미지를 로컬에 임시 생성한 뒤, AI의 이미지 분석(멀티모달 시각 분석)을 거쳐 시각적 패턴, 이상치, 착시 효과 등을 종합적으로 판단하여 해석을 작성/보완해야 한다.** 임시 생성된 플롯 이미지는 모든 분석 작업이 종료된 후 반드시 즉시 삭제하여 디렉토리를 청결하게 유지한다.
- **반복 분석 및 시각화 개선 루프 (필수)**: EDA 수행 중 결과가 사전 가설과 다르거나, 통계 결과와 플롯의 시각적 패턴이 맞지 않거나, 플롯이 작거나 겹치거나 축/범례/라벨 문제로 해석이 어렵거나, 이상치·스케일·결측 처리 때문에 착시가 의심되는 경우에는 해석을 확정하지 않는다. 반드시 1) 원자료/중간 집계 재확인, 2) 검정 코드와 표본 수 재검증, 3) 플롯 크기·축 스케일·정렬·색상·라벨·facet 구성을 수정, 4) 임시 플롯 재생성 및 이미지 직접 확인, 5) 필요 시 코드와 해석을 다시 수정하는 과정을 반복한다. 이 반복은 플롯과 통계표가 함께 해석 가능한 수준이 될 때까지 수행하며, 최종 노트북에는 실패한 중간 시도나 불필요한 디버깅 출력은 남기지 않는다.
- **기본 답변 언어 원칙 (필수)**: AI 에이전트는 사용자와 대화하고 지시를 수행하거나 작업 결과를 답변할 때 **반드시 한국어(Korean)로만 대답해야 한다.**
